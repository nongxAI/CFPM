...
class DeterministicGenerator:
    ...
    def get_global_size_and_indices(self, size):
        ...
        indices = th.arange(self.done_samples + self.rank, self.done_samples + self.world_size * int(size[0]), self.world_size)
...
class DeterministicIndividualGenerator:
    ...
    def randn(self, *size, dtype=th.float, device='cpu'):
        ...
        generator = self.get_generator(device)
        return th.cat([th.randn(*size, generator=generator[i], dtype=dtype, device=device) for i in indices], dim=0)

    def randint(self, low, high, size, dtype=th.long, device='cpu'):
        size, indices = self.get_size_and_indices(size)
        generator = self.get_generator(device)
        return th.cat([th.randint(low, high, generator=generator[i], size=size, dtype=dtype, device=device) for i in indices], dim=0)
